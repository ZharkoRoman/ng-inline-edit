      ...
      function ($compile, InlineEditConfig, InlineEditConstants) {
        return {
          ...
          link: function (scope, element, attrs) {
            ...
            // text
            var onClickConfig = {
              required: attrs.hasOwnProperty('inlineEditOnClick') || InlineEditConfig.editOnClick,
              template: 'ng-click="editText()"'
            };
            var onDblConfig = {
              required: attrs.hasOwnProperty('inlineEditOnDblClick') || InlineEditConfig.editOnDblClick,
              template: 'ng-dblclick="editText()"'
            };
            var editableConfigs = [onClickConfig, onDblConfig];


            if (onClickConfig.required && onDblConfig.required) {
              throw new SyntaxError('There should be only one event to edit. Specify one of onClick or onDblClick');
            }
            var editableTemplate = editableConfigs.find(function (config) {
              return config.required
            }).template;

            var textTemplate = '<span class="ng-inline-edit__text" ' +
            ...
            innerContainer.append(angular.element(textTemplate));
            ...
            if (inlineEditBtnEdit) {
              innerContainer.append(angular.element(
                '<a class="ng-inline-edit__button ng-inline-edit__button--edit btn btn-sm edit-title" ' +
                'ng-if="!editMode" ' +
                'ng-click="editText()">' +
